...
class API:
    ...
    def upload(self, path: Path, fields: dict[str, str], progress=None, attempts: int = 3):
        """Upload a file, retrying fresh connections after transient socket loss.

        The request id remains stable across attempts so the server can return
        the first successful result instead of adding the same track twice
        ...
        for attempt in range(1, max(1, attempts) + 1):
            try:
                return self._upload_once(path, fields, progress, upload_id)
            except APIError as exc:
                last_error = exc
                if not getattr(exc, "transient", False) or attempt >= attempts:
                    raise
                time.sleep(min(2 ** (attempt - 1), 4))
        raise last_error  # pragma: no cover

    def _upload_once(self, path: Path, fields: dict[str, str], progress, upload_id: str):
        ...
        conn.putheader("X-Upload-ID", upload_id)
        conn.endheaders()
        sent = 0
        send_error = None
        try:
            ...
            # The server can reject an upload as soon as it has read the
            # multipart headers. Surface any server response before raising socket errors.
            send_error = exc
        raw = ""
        response = None
        try:
            ...
            if send_error is not None:
                error = APIError(f"Upload connection closed unexpectedly: {send_error}")
                error.transient = True
                raise error from exc
            error = APIError(f"Upload failed: {exc}")
            error.transient = isinstance(exc, (ConnectionError, TimeoutError, OSError))
            raise error from exc
        finally:
            try:
                conn.close()
        ...
        if send_error is not None and (not payload or not payload.get("ok")):
            raise APIError(f"Upload connection closed early: {send_error}")
        return payload
...
class DJApp(tk.Tk):
    ...
    def show_dashboard(self):
        ...
        self.tab_builders = {
            ...
            "integrity": self.build_catalog_integrity,
            "ads": self.build_ads,
    ...
    def set_takeover_status(self,status):
        ...
        try:
            ...
            if status=="published" and result.get("takeover"):
                def send_alert(tid):
                    try:
                        token = getattr(self.api, "token", "") or getattr(self.api, "password", "")
                        headers = {"Content-Type": "application/json"}
                        if token:
                            headers["Authorization"] = f"Bearer {token}"
                        req = urllib.request.Request("https://allthings140radio.online/api/public/takeover-alert", data=json.dumps({"id": tid}).encode(), method="POST", headers=headers)
                        urllib.request.urlopen(req, timeout=20).read()
                    except Exception:
                        pass
                threading.Thread(target=send_alert, args=(takeover_id,), daemon=True).start()
    ...
    def build_library(self):
        ...
        ttk.Button(tools, text="Review Duplicates", command=self.detect_library_duplicates).pack(side="right")
    ...
    def build_catalog_integrity(self):
        f = self.tabs["integrity"]
        head = tk.Frame(f, bg="#100b18"); head.pack(fill="x")
        tk.Label(head, text="CATALOG INTEGRITY", bg="#100b18", fg="#efe5f8", font=("Sans", 14, "bold")).pack(side="left")
        ttk.Button(head, text="SCAN AGAIN", command=self.refresh_catalog_integrity).pack(side="right")
        self.integrity_filter = ttk.Combobox(head, state="readonly", width=28, values=(
            "ALL", "PLAYABLE", "STORED_NOT_PLAYBACK_READY", "MISSING_AUDIO", "DISABLED", "UNAPPROVED", "RIGHTS_BLOCKED"
        ))
        self.integrity_filter.set("ALL"); self.integrity_filter.pack(side="right", padx=8)
        self.integrity_filter.bind("<<ComboboxSelected>>", lambda _event: self.render_catalog_integrity())
        self.integrity_cards = tk.Label(f, text="Loading…", bg="#171020", fg="#d8cbe2", justify="left", anchor="w", padx=16, pady=14)
        self.integrity_cards.pack(fill="x", pady=(14, 10))
        self.integrity_tree = ttk.Treeview(f, columns=("id", "status", "artist", "track", "filename", "path"), show="headings")
        for key, title, width in (("id", "ID", 60), ("status", "Status", 190), ("artist", "Artist", 160), ("track", "Track", 250), ("filename", "Expected filename", 330), ("path", "Resolved / expected path", 390)):
            self.integrity_tree.heading(key, text=title); self.integrity_tree.column(key, width=width, anchor="w")
        self.integrity_tree.pack(fill="both", expand=True)
        self.integrity_status = tk.Label(f, text="Read-only scan. Nothing is automatically deleted.", bg="#100b18", fg="#9f91aa")
        self.integrity_status.pack(anchor="w", pady=(8, 0))
        self.integrity_rows = []
        self.refresh_catalog_integrity()

    def refresh_catalog_integrity(self):
        if not hasattr(self, "integrity_tree"):
            return
        self.integrity_status.config(text="Scanning catalog, master storage, emergency mirror, and playback readiness…", fg="#d8b7ff")
        def worker():
            try:
                report = self.api.request("GET", "/api/catalog-integrity", timeout=180)
                self.post_ui(lambda: self._apply_catalog_integrity(report, None))
            except Exception as exc:
                self.post_ui(lambda: self._apply_catalog_integrity({}, str(exc)))
        threading.Thread(target=worker, daemon=True).start()

    def _apply_catalog_integrity(self, report, error):
        if error:
            self.integrity_status.config(text=f"Integrity scan failed: {error}", fg="#ff9ca8"); return
        self.integrity_report = report
        self.integrity_rows = list(report.get("records", []))
        c = report.get("counts", {})
        self.integrity_cards.config(text=(
            f"Health: {str(report.get('health', 'unknown')).upper()}     Total Catalog: {c.get('total_catalog', 0)}     "
            f"Playback Ready: {c.get('playback_ready', 0)}     Stored / Awaiting Local Admission: {c.get('stored_not_playback_ready', 0)}\n"
            f"Missing Approved Audio: {c.get('missing_approved_audio', 0)}     Disabled: {c.get('disabled', 0)}     "
            f"Unapproved: {c.get('unapproved', 0)}     Rights Blocked: {c.get('rights_blocked', 0)}     "
            f"Shared Filename: {c.get('shared_filename_records', 0)}     Exact Duplicate Groups: {c.get('exact_duplicate_groups', 0)}     "
            f"Probable / Metadata Review: {c.get('probable_duplicate_groups', 0)}     Orphan Files: {c.get('orphan_files', 0)}"
        ), fg="#ff9ca8" if report.get("health") != "healthy" else "#7ce5ad")
        self.integrity_status.config(text=f"Scan completed {time.strftime('%Y-%m-%d %H:%M:%S')}. Read-only; no records or files changed.", fg="#7ce5ad")
        self.render_catalog_integrity()

    def render_catalog_integrity(self):
        if not hasattr(self, "integrity_tree"):
            return
        selected = self.integrity_filter.get() or "ALL"
        self.integrity_tree.delete(*self.integrity_tree.get_children())
        for row in self.integrity_rows:
            if selected != "ALL" and row.get("status") != selected:
                continue
            resolved = row.get("playback", {}).get("path") if row.get("playback_ready") else row.get("expected_path", "")
            self.integrity_tree.insert("", "end", iid=str(row["id"]), values=(row["id"], row.get("status"), row.get("artist"), row.get("title"), row.get("filename"), resolved))
    ...
    def show_duplicate_results(self, result):
        ...
        self.upload_status.config(text=f"Duplicate review found {len(groups)} group(s). No files or records were changed.")
        dialog = tk.Toplevel(self)
        ...
        tk.Label(dialog, text="Only hashes prove exact audio. Metadata matches are review-only and are never automatically removed.", bg="#100b18", fg="#9f91aa").pack(anchor="w", pady=(4, 12))
        text = tk.Text(dialog, bg="#08040d", fg="#eee4f5", insertbackground="white", relief="flat", padx=14, pady=12, wrap="word")
        text.pack(fill="both", expand=True)
        for index, group in enumerate(groups, 1):
            text.insert("end", f"{index}. {group.get('classification', group['reason'])}\n")
            for track in group.get("tracks", []):
                text.insert("end", f"   ID {track['id']}  •  {track.get('artist', '')} — {track.get('title', '')}\n   {track.get('filename', '')}\n   size={track.get('size_bytes')} duration={track.get('duration')} hash={track.get('sha256') or 'not indexed'}\n   path={track.get('resolved_path', '')}\n")
            text.insert("end", "\n")
        text.configure(state="disabled")
        actions = tk.Frame(dialog, bg="#100b18")
        actions.pack(fill="x", pady=(12, 0))
        tk.Label(actions, text="REVIEW ONLY — automatic cleanup disabled", bg="#100b18", fg="#ff9ca8").pack(side="left")
    ...
    def remove_duplicate_copies(self, groups, dialog=None, confirm=True):
        messagebox.showwarning("Automatic cleanup disabled", "Duplicate candidates must be reviewed in Catalog Integrity. This action cannot delete catalog rows or audio files.", parent=dialog or self)
        ...
        def worker():
            ...
            for index, audio_path in enumerate(files, 1):
                try:
                    ...
                    result = self.api.upload(audio_path, {
            ...
            def done():
                ...
                parts = []
                if completed:
                    parts.append(f"Uploaded {completed} new file(s)")
                if duplicates:
                    parts.append(f"{duplicates} already in catalog")
                summary_lead = ", ".join(parts) if parts else "0 files uploaded"
                message = f"{summary_lead} (out of {len(files)} file(s)). AutoDJ is refreshing."
                if failed:
                    message += "\n\nFailed:\n" + "\n".join(failed[:8])
    ...
    def build_review(self):
        ...
        for key,title,width in (("title","Artist / Audio",330),("genre","Type / BPM",180),("status","Status",160),("url","Review link",360)):
        ...
        tk.Label(f, text="Public track and prerecorded-mix submissions appear here. Approve only after verifying rights and audio quality; approved files belong in the station's Google Drive library.", bg="#100b18", fg="#a99ab3", wraplength=850, justify="left").pack(anchor="w")
    ...
    def build_support(self):
        f=self.tabs["support"]
        tk.Label(f,text="SUPPORT DASHBOARD",bg="#100b18",fg="#e0b3ff",font=("Sans",16,"bold")).pack(anchor="w")
        tk.Label(f,text="Totals include only Stripe payments confirmed by webhook, minus verified refunds.",bg="#100b18",fg="#9f91aa").pack(anchor="w",pady=(3,12))
        self.support_metrics=tk.Frame(f,bg="#100b18");self.support_metrics.pack(fill="x")
        self.support_metric_labels={}
        for key,title in (("today","TODAY"),("week","THIS WEEK"),("month","THIS MONTH"),("all_time","ALL TIME")):
            box=tk.Frame(self.support_metrics,bg="#171020",highlightbackground="#633080",highlightthickness=1,padx=14,pady=12);box.pack(side="left",fill="x",expand=True,padx=(0,8))
            tk.Label(box,text=title,bg="#171020",fg="#b79ac5",font=("Sans",8,"bold")).pack(anchor="w")
            label=tk.Label(box,text="$0 · 0 supporters",bg="#171020",fg="#fff",font=("Sans",13,"bold"));label.pack(anchor="w",pady=(5,0));self.support_metric_labels[key]=label
        settings=tk.LabelFrame(f,text=" SUPPORT GOAL + PUBLIC DISPLAY ",bg="#100b18",fg="#d7b9ff",padx=12,pady=10);settings.pack(fill="x",pady=14)
        row=tk.Frame(settings,bg="#100b18");row.pack(fill="x")
        self.support_goal_title=ttk.Entry(row);self.support_goal_title.pack(side="left",fill="x",expand=True)
        self.support_goal_target=ttk.Entry(row,width=14);self.support_goal_target.pack(side="left",padx=8)
        self.support_goal_description=ttk.Entry(settings);self.support_goal_description.pack(fill="x",pady=(8,0))
        self.support_goal_enabled=tk.BooleanVar();self.support_feed_enabled=tk.BooleanVar();self.support_live_enabled=tk.BooleanVar()
        checks=tk.Frame(settings,bg="#100b18");checks.pack(fill="x",pady=(8,0))
        for text,var in (("Show goal",self.support_goal_enabled),("Show public feed",self.support_feed_enabled),("Live tip animations",self.support_live_enabled)):
            tk.Checkbutton(checks,text=text,variable=var,bg="#100b18",fg="#eee",selectcolor="#281a33",activebackground="#100b18").pack(side="left",padx=(0,16))
        ttk.Button(checks,text="SAVE SUPPORT SETTINGS",command=self.save_support_settings).pack(side="right")
        self.support_tree=ttk.Treeview(f,columns=("date","amount","name","message","provider","status"),show="headings",height=12)
        for key,title,width in (("date","Date",140),("amount","Amount",80),("name","Public name",130),("message","Message",310),("provider","Provider",80),("status","Status",110)):
            self.support_tree.heading(key,text=title);self.support_tree.column(key,width=width)
        self.support_tree.pack(fill="both",expand=True)
        actions=tk.Frame(f,bg="#100b18");actions.pack(fill="x",pady=(8,0))
        ttk.Button(actions,text="REFRESH",command=self.refresh_support).pack(side="left")
        ttk.Button(actions,text="HIDE PUBLIC ENTRY",command=lambda:self.moderate_support(False,False)).pack(side="left",padx=8)
        ttk.Button(actions,text="HIDE MESSAGE ONLY",command=lambda:self.moderate_support(True,False)).pack(side="left")
        ttk.Button(actions,text="RESTORE PUBLIC ENTRY",command=lambda:self.moderate_support(True,True)).pack(side="left",padx=8)
        self.support_rows={};self.refresh_support()

    def refresh_support(self):
        try:data=self.api.request("GET","/api/support/dashboard")
        except APIError as exc:return messagebox.showerror("Support dashboard unavailable",str(exc),parent=self)
        for key,value in data.get("metrics",{}).items():
            if key in self.support_metric_labels:self.support_metric_labels[key].config(text=f"${value.get('total_cents',0)/100:,.2f} · {value.get('supporters',0)} supporters")
        settings=data.get("settings",{});self.support_goal_title.delete(0,"end");self.support_goal_title.insert(0,settings.get("goal_title",""));self.support_goal_target.delete(0,"end");self.support_goal_target.insert(0,f"{settings.get('goal_target_cents',0)/100:.2f}");self.support_goal_description.delete(0,"end");self.support_goal_description.insert(0,settings.get("goal_description",""));self.support_goal_enabled.set(bool(settings.get("goal_enabled")));self.support_feed_enabled.set(bool(settings.get("public_feed_enabled")));self.support_live_enabled.set(bool(settings.get("live_notifications_enabled")))
        self.support_tree.delete(*self.support_tree.get_children());self.support_rows={}
        for row in data.get("recent",[]):
            iid=str(row["id"]);self.support_rows[iid]=row;stamp=row.get("completed_at") or row.get("created_at") or 0;date=dt.datetime.fromtimestamp(stamp).strftime("%Y-%m-%d %H:%M")
            status=row.get("payment_status","")+(f" · refunded ${row.get('refunded_cents',0)/100:.2f}" if row.get("refunded_cents") else "")
            self.support_tree.insert("","end",iid=iid,values=(date,f"${row.get('amount_cents',0)/100:.2f}",row.get("display_name","Anonymous"),row.get("supporter_message","") if row.get("message_visible") else "[hidden]",row.get("provider",""),status))

    def save_support_settings(self):
        try:target=round(float(self.support_goal_target.get())*100)
        except ValueError:return messagebox.showerror("Invalid goal","Enter the target in dollars, such as 150.00",parent=self)
        data={"goal_title":self.support_goal_title.get(),"goal_description":self.support_goal_description.get(),"goal_target_cents":target,"goal_start":0,"goal_reset_mode":"never","goal_enabled":self.support_goal_enabled.get(),"public_feed_enabled":self.support_feed_enabled.get(),"live_notifications_enabled":self.support_live_enabled.get()}
        try:self.api.request("POST","/api/support/settings",data);messagebox.showinfo("Saved","Support settings updated.",parent=self);self.refresh_support()
        except APIError as exc:messagebox.showerror("Could not save",str(exc),parent=self)

    def moderate_support(self,public_visible,message_visible):
        selected=self.support_tree.selection()
        if not selected:return messagebox.showinfo("Select support","Select a support entry first.",parent=self)
        try:self.api.request("POST","/api/support/moderate",{"id":int(selected[0]),"public_visible":public_visible,"message_visible":message_visible});self.refresh_support()
        except APIError as exc:messagebox.showerror("Could not update",str(exc),parent=self)
            ...
            if hasattr(self, "health_labels"):
                ...
                if hasattr(self, "ops_summary"):
                    ...
                    self.ops_summary.config(text=(
                        ...
                        f"HOT CACHE  {str(cache.get('status', 'UNKNOWN')).upper()}     READY  {cache.get('minutes_ready', 0)} MIN / {cache.get('tracks_ready', 0)} TRACKS"
                    ))
                states = {
                    "SERVER": ("HEALTHY", True),
