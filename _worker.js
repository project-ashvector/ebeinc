const RADIO_HOST = "allthings140radio.online";
...
// ── Visual Routing — persistent mode control ──────────────────────────
...
async function readRoutingState(env) {
  if (env.VISUALS_ROUTING_KV) {
    ...
    if (raw && typeof raw === "object") {
      return { chat: raw.chat || "legacy", visuals: raw.visuals || "legacy" };
    }
  }
...
async function handleVisualRoutingPost(request, env, ctx) {
  ...
  const next = {
    chat: chatMode || current.chat,
    visuals: visualsMode || current.visuals,
  };
...
function radioAssetPath(pathname) {
  if (pathname === "/radio" || pathname === "/radio/") return "/radio/";
  if (pathname.startsWith("/radio/")) return pathname;
  if (pathname === "/") return "/radio/";
  return `/radio${pathname}`;
}

export default {
  async fetch(request, env, ctx) {
    const url = new URL(request.url);
    const isRadioHost = url.hostname === RADIO_HOST;
    const isLegacyRadioPath = url.pathname === "/liveradio" || url.pathname.startsWith("/liveradio/");

    if (isRadioHost && url.pathname === "/obs/live.mp3" && (request.method === "GET" || request.method === "HEAD")) return obsAudioStream(request, env);

    if (isRadioHost && url.pathname === "/api/public/newsletter/webhook" && request.method === "POST") return handleMailchimpWebhook(request, env);

    if (isRadioHost && url.pathname === "/api/public/newsletter" && request.method === "POST") {
      const response = await mailchimpSignup(request.clone(), env);
      if (response) return response;
    }
    if (isRadioHost && url.pathname === "/api/public/newsletter/status" && request.method === "GET") {
    ...
    // Visual routing endpoints (public read, authenticated write)
    if (isRadioHost && url.pathname === "/api/visual-routing" && request.method === "GET") {
      return handleVisualRoutingGet(request, env);
    }
    if (isRadioHost && url.pathname === "/api/visual-routing" && request.method === "POST") {
      return handleVisualRoutingPost(request, env, ctx);
    }
    if (isRadioHost && url.pathname === "/api/visual-health" && request.method === "GET") {
      return handleVisualHealth(request, env);
    }

    if (isRadioHost && url.pathname === "/api/public/takeover-alert" && request.method === "POST") return handleTakeoverAlert(request, env, ctx);

    if (isRadioHost || isLegacyRadioPath) {
      if (isLegacyRadioPath) {
        url.pathname = url.pathname.replace(/^\/liveradio/, "") || "/";
      }
      ...
      url.pathname = radioAssetPath(url.pathname);
      const response = await env.ASSETS.fetch(new Request(url, request));
      if (url.pathname === "/radio/sw-v47.js") {
